...
namespace BeyazEsyaWeb.Controllers
{
	public class ListProductController : Controller
	{
		...
		public ListProductController(IConfiguration configuration)
		{
		...
		[HttpPost]
		public IActionResult DeleteProduct([FromBody] Products urun)
		{
...

			{
				using (var connection = new SqlConnection(connectionString))
				{
					...
					connection.Open();
					string sqlQuery = "DELETE FROM Products WHERE Id=@urunid";
					using (var command = new SqlCommand(sqlQuery, connection))
					{						
						command.Parameters.AddWithValue("@urunid", urun.id);
